...
$stmt = $db -> prepare('SELECT * FROM "Kutuphane" WHERE kutuphane_id=:kutuphane_id');
$stmt -> execute([
    'kutuphane_id' => $_GET['kutuphane_id']
]);
$kutuphane = $stmt -> fetch(PDO::FETCH_ASSOC);

$stmt2 = $db -> prepare('SELECT * FROM "Adresler" WHERE adres_id=:adres_id');
$stmt2 -> execute([
    'adres_id' => $kutuphane['adres_id']
]);
    ...
    <div class="page-content">
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h6 class="card-title">Kütüphane Düzenle</h6>

                        <form method="POST" action="process.php">

                            <input type="hidden" name="kutuphane_id" value="<?php echo $_GET['kutuphane_id'] ?>">
                            ...
                            <div class="row">
                                <div class="mb-3 col-sm-12">
                                    ...
                                    <input required type="text" class="form-control" id="exampleInputText1" name="kutuphane_isim"
                                           value="<?php echo $kutuphane['kutuphaneAd'] ?>" placeholder="Kütüphane İsmi Giriniz">
                                </div>
                            </div>
                            ...
                            <button class="btn btn-primary" type="submit" name="kutuphaneduzenle">Gönder</button>
                        </form>
                    </div>
                </div>
            </div>
